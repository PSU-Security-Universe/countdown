...
func (ctx *mutator) insertRefcountCall() bool {
	p, r := ctx.p, ctx.r
	if len(p.Calls) >= ctx.ncalls {
		return false
	}
	if len(p.RefcountCallCandidate) == 0 {
		return false
	}
	chosen_call_pos := r.Int31n(int32(len(p.RefcountCallCandidate)))
	chosen_call_idx_in_syz := p.RefcountCallCandidate[chosen_call_pos]

	idx := r.biasedRand(len(p.Calls)+1, 5) // insert position
	var c *Call
	if idx < len(p.Calls) {
		c = p.Calls[idx]
	}
	s := analyze(ctx.ct, ctx.corpus, p, c)
	calls := r.generateCallByIndex(s, p, int(chosen_call_idx_in_syz))
	p.insertBefore(c, calls)
	for len(p.Calls) > ctx.ncalls {
		p.RemoveCall(idx)
	}
