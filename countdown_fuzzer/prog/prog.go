...
type CD_sum_op struct {
	Call_idx_in_prog int32
	Hash             uint32
}

type CD_sum_res struct {
	Refcnt_delta_sum int32
	After_value      int32
	Call_idx_in_syz  int32
}

type One_turn_res struct {
	One_turn_findings map[CD_sum_op]CD_sum_res
}
...
type Prog struct {
	...
	RefcountCallCandidate []int32
	CD_one_turn_findings map[CD_sum_op]CD_sum_res
	CD_COMMENT           string
	CD_flags int
	CD_exec_source int // [1] new prog, [2] triage, [0] default.
	CD_callidx_signal_map map[int32][]uint32
	Dup DupInfo
}

type DupInfo struct {
	CD_dup_syscall_nr             int32
	CD_dup_target_hash            uint32
	CD_dup_original_refcnt_change int32
	CD_dup_mode                   int
	CD_dup_scope_start            int
	CD_dup_scope_end              int
}
...
// DuplicateCall:mode: [0] append at original position, [1] append at the end
func (p *Prog) DuplicateCall(idx_in_prog int, times int, mode int) *Prog {

	newP := p.Clone()

	var interesting_call = newP.Calls[idx_in_prog]
	var newCalls []*Call

	if mode == 0 {
		newCalls = append(newCalls, newP.Calls[:idx_in_prog]...)
		for i := 0; i < times; i++ {
			new_call := MakeCall((*interesting_call).Meta, (*interesting_call).Args)
			newCalls = append(newCalls, new_call)
		}
		if idx_in_prog < len(newP.Calls) {
			newCalls = append(newCalls, newP.Calls[idx_in_prog:]...)
		}
	} else if mode == 1 {
		newCalls = append(newCalls, newP.Calls...)
		for i := 0; i < times; i++ {
			new_call := MakeCall((*interesting_call).Meta, (*interesting_call).Args)
			newCalls = append(newCalls, new_call)
		}
	...
	newP.Calls = newCalls
	newP.CD_COMMENT = "CountDown duplicate"
	newP.CD_flags = p.CD_flags
	newP.Dup.CD_dup_mode = mode

	newP.Dup.CD_dup_scope_start = idx_in_prog
	newP.Dup.CD_dup_scope_end = idx_in_prog + times
	return newP
}
