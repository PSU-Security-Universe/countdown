...
// CountDown this function only copied a new runs for future updates.
// the other members should not change during fuzzing.
func (ct *ChoiceTable) CD_CopyChoiceTable() *ChoiceTable {
	run := make([][]int32, len(ct.runs))
	for i := range run {
		if ct.runs[i] != nil {
			run[i] = make([]int32, len(ct.runs))
			copy(run[i], ct.runs[i])
		}
	}
	return &ChoiceTable{ct.target, run, ct.calls, ct.noGenerateCalls, ct.Prios, ct.LogPrios}
}
...
func (target *Target) BuildChoiceTable(corpus []*Prog, enabled map[*Syscall]bool) *ChoiceTable {
	...
	log_prios := make([][]int32, len(prios)) // added not test now
	for i := range log_prios {
		log_prios[i] = make([]int32, len(prios))
		for j := range log_prios[i] {
			log_prios[i][j] = int32(math.Log2(float64(prios[i][j]) + 1))
		}
	}
	return &ChoiceTable{target, run, generatableCalls, noGenerateCalls, prios, log_prios}
}

func (ct *ChoiceTable) CD_update_runs_from_prio(prios [][]int32) {
	normalizePrio(prios)
	runs := make([][]int32, len(prios))
	for i := range runs {
		if !ct.Enabled(i) || !ct.Generatable(i) {
			continue
		}
		runs[i] = make([]int32, len(prios))
		var sum int32
		for j := range runs[i] {
			if ct.Enabled(j) && ct.Generatable(j) {
				sum += prios[i][j]
			}
			runs[i][j] = sum
		}
