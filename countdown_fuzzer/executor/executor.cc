...
int cd_executor_id = -1;

int get_current_executor_id(char* prog_name) {
    const char *delim = ".";
    char *p;

	char* executor_id_str = NULL;
    p = strtok(prog_name, delim);
    while (p != NULL)
    {
		executor_id_str = p;
        p = strtok(NULL, delim);
    }
	int executor_id = atoi(executor_id_str);
	return executor_id;
}
...
int main(int argc, char** argv)
{
	...
	cd_executor_id = get_current_executor_id(argv[0]);
...
void cd_ctl(thread_t* th, const call_t* call)
{
	/////////////////////////////////////////////////////////////////////////////////////
	syscall(1006, cd_executor_id, th->call_index); // set used_array; enable logging for current task;
	NONFAILING(th->res = execute_syscall(call, th->args)); // writing to kernel global table
	syscall(1004); 	// just disable logging
	/////////////////////////////////////////////////////////////////////////////////////
}
